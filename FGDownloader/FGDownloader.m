...
-(void)getGrowthSize
{
    uint32_t size=(uint32_t)[[[[NSFileManager defaultManager] attributesOfItemAtPath:_destination_path error:nil] objectForKey:NSFileSize] integerValue];
...
-(void)downloadWithUrlString:(NSString *)urlString toPath:(NSString *)destinationPath process:(ProcessHandle)process completion:(CompletionHandle)completion failure:(FailureHandle)failure
{
    if(urlString&&destinationPath)
    {
        ...
        if(fileExist)
        {
            uint32_t length=(uint32_t)[[[fileManager attributesOfItemAtPath:destinationPath error:nil] objectForKey:NSFileSize] integerValue];
            NSString *rangeString=[NSString stringWithFormat:@"bytes=%u-",length];
...
+(NSString *)filesSize:(NSString *)url
{
    ...
    uint32_t totalLength=(uint32_t)[usd integerForKey:totalLebgthKey];
    if(totalLength==0)
    {
        return @"0.00K/0.00K";
    }
    ...
    uint32_t currentLength=progress*totalLength;
...
-(void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    NSUserDefaults *usd=[NSUserDefaults standardUserDefaults];
    uint32_t totalLength=(uint32_t)[usd integerForKey:key];
    if(totalLength==0)
    {
...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    uint32_t freeSpace=[self systemFreeSpace];
    ...
    uint32_t length=(uint32_t)[[[[NSFileManager defaultManager] attributesOfItemAtPath:_destination_path error:nil] objectForKey:NSFileSize] integerValue];
    NSString *key=[NSString stringWithFormat:@"%@totalLength",_url_string];
    uint32_t totalLength=(uint32_t)[[NSUserDefaults standardUserDefaults] integerForKey:key];
 ...
 * @prama length  文件大小
 */
+(NSString *)convertSize:(uint32_t)length
{
    if(length<1024)
        return [NSString stringWithFormat:@"%uB",length];
